...
const App = (function () {
  ...
  function renderPortal() {
    ...
    ['volunteer', 'released'].forEach((role, i) => {
      const cfg = R[role];
      bottomRow.appendChild(portalCard(role, cfg, i + 4));
    });

    const grid = el('div', { class: 'portal-layout' }, topRow, centerRow, bottomRow);
  ...
  function dashboardShell(user, pageContent) {
    const actions = {
      ...
      judicial: [{ icon: '⚠️', label: '开展风险评级', page: 'risk' }, { icon: '🤖', label: '查看数据分析', page: 'analysis' }],
      released: [{ icon: '✏️', label: '更新我的信息', page: 'update' }, { icon: '❓', label: '提交疑问', page: 'ask' }]
    }[user.role] || [];
    const messages = { police: '及时完善档案信息，为后续衔接提供可靠依据。', prison: '关注临释人员动态，确保交接环节顺畅完成。', judicial: '聚焦待办事项，为回归社会提供持续支持。', released: '保持信息更新，主动获取帮扶与就业资源。' };
    return el('div', {},
  ...
  function jobsManagePage(user) {
    ...
    const frag = el('div', { class: 'card' },
      ...
      el('div', { class: 'hint' }, '筛选并上传由人社部门发送的企业招聘信息。'),
      jobs.length ? table(
